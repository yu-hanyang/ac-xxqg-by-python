...
def back_my_point(web):
    ...
    web.find_element_by_xpath('//*[@id="page-main"]/div/section/div/div[4]').click()
    sleep()
    web.close()
    web.switch_to.window(web.window_handles[0])
    sleep()
...
def show_Qrcode(path):
    # 读取图片
    img = cv.imread(path)
    img = cv.resize(img, (500, 500))
    # cv.namedWindow('result', cv.WINDOW_NORMAL)
    # 显示图片
    cv.imshow('plesae load in in 10 second', img)
    # 等待
    cv.waitKey(10000)
    # 释放内存
...
def main():
    ...
    with open('QRcode.png', mode='wb+') as p:
        img = get_QRcode(web)
        p.write(img)
